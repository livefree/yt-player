...
## 2026-04-07 任务启动：SEQ-20260407-33 面板系统回退与统一重建

### 本次变更
- 新增 `SEQ-20260407-33`
- 停止对 speed / settings 弹窗继续做局部修补，改为直接回退到 episodes 面板模型统一重建

### 当前目标
- 删除 speed / settings 的旧独立弹窗壳层与尺寸逻辑
- 让 speed / settings / episodes 真正复用同一层 `panel shell + scroller + content-driven` 契约
- 将 settings 面板临时收口为占位菜单，移除旧的 quality / subtitles 多级菜单状态

### 完成情况（2026-04-07 17:46）
- 新增 `src/player/components/PanelShell.tsx`，统一承载 `placement / sizing / scroller / content-driven` 面板壳层；`EpisodesPanel` 已切到同一层消费
- `src/player/components/SpeedPanel.tsx` 已删除旧的独立 popup 壳层与 header 逻辑，改为通过共享 `PanelShell` 渲染 slider + preset 内容
- `src/player/components/SettingsPanel.tsx` 已删除旧的 quality / subtitles 多级菜单逻辑，改为 flat placeholder menu，暂时提供 `Loop playback / Autoplay next / Sleep timer / Caption style` 四行占位入口
- `src/player/types.ts` 内部 `Panel` 状态已收口为 `"settings" | "speed" | null`，移除已废弃的 `quality / subtitles` 弹窗状态
- `src/player/Player.module.css` 已移除旧的 `.ytpSettingsMenu` / speed 独立 popup 壳层职责，统一到 `.ytpPanelPopup` 契约；fullscreen / compact / top-right / bottom-right 的面板位置规则开始由共享壳层承载
- `src/test/Player.test.tsx` 已更新为新的统一面板契约测试：settings 使用 placeholder rows，speed 不再依赖旧 header，多级 settings 断言已删除
- 验证通过：`npm run typecheck`、`npm run lint`、`npm test`、`npm run build`
