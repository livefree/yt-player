...
## [SEQ-20260407-33] YTPlayer 面板系统回退与统一重建

- **状态**：✅ 完成
- **创建时间**：2026-04-07 15:05
- **最后更新时间**：2026-04-07 17:46
- **目标**：停止对 speed / settings 弹窗的局部修补，删除现有独立弹窗系统，改为基于 episodes 面板模型统一重建
- **范围**：`src/player/`、`src/test/`、`task.md`、`DEVLOG.md`、`docs/changelog.md`
- **依赖**：延续现有 layout / overlay / slot 架构；不改公开 API；settings 面板先用占位菜单替代旧的多级逻辑

### 任务列表（按执行顺序）

1. PLAYER-64 — 以 episodes 面板模型重建 speed / settings popup（状态：✅）
   - 创建时间：2026-04-07 15:05
   - 计划开始：2026-04-07 15:05
   - 实际开始：2026-04-07 15:05
   - 完成时间：2026-04-07 17:46
   - 验收要点：删除 speed / settings 旧弹窗壳层和独立尺寸策略；两者改用与 episodes 一致的 panel shell / scroller / content-driven 行为

2. PLAYER-65 — 将 settings 简化为占位菜单并移除旧多级状态依赖（状态：✅）
   - 创建时间：2026-04-07 15:05
   - 计划开始：PLAYER-64 完成后
   - 实际开始：2026-04-07 15:05
   - 完成时间：2026-04-07 17:46
   - 验收要点：settings 面板改为平铺占位行；移除 quality / subtitles 子面板依赖；不再保留旧的嵌套菜单弹窗逻辑

3. PLAYER-66 — 更新统一面板契约测试（状态：✅）
   - 创建时间：2026-04-07 15:05
   - 计划开始：PLAYER-65 完成后
   - 实际开始：2026-04-07 15:05
   - 完成时间：2026-04-07 17:46
   - 验收要点：更新 speed / settings / episodes 共享面板结构测试；移除已废弃的旧 settings 多级菜单断言
