...
## 2026-04-07 任务启动：SEQ-20260407-37 空 settings 入口禁用

### 本次变更
- 新增 `SEQ-20260407-37`
- 开始收口空 settings 入口：保留按钮，但在无可用设置项时灰化禁用，并阻断空弹窗路径

### 当前目标
- 让 settings 按钮在空状态下保持可见但不可交互
- 保持已有 settings overlay 行为仅在真实设置项存在时成立

### 完成情况（2026-04-07 19:36）
- `src/player/Player.tsx` 已新增 `hasSettingsContent` 判定；qualities / subtitles 都为空时，settings 按钮继续渲染，但会进入 disabled / aria-disabled 状态，并阻止点击打开空弹窗
- `src/player/Player.tsx` 已补自动收口逻辑：若 settings 已打开而可用设置项随后变为空，会立即关闭旧 settings 面板
- `src/player/Player.module.css` 已为 disabled settings 按钮补充灰化与 tooltip 抑制样式，避免空入口继续表现为可交互按钮
- `src/test/Player.test.tsx` 已补“空 settings 仍显示但禁用”回归测试，并修正 3 条 overlay manager 测试为“存在 settings 内容时才验证面板遮罩行为”，当前回归测试总数为 86
- 验证通过：`npm run typecheck`、`npm run lint`、`npm test`、`npm run build`

## 2026-04-07 方案冻结：SEQ-20260407-38 settings / speed 重建前置规划

### 本次变更
- 开始将 `SEQ-20260407-38` 从“旧 panel 拆除”调整为“文档校准 + 布局规划 + 重建前置”
- 当前仅更新文档 contract，不推进实现代码

### 已冻结的布局决策
- `AirPlay + PiP` 以播放器右上为最高优先级落位
- 右下在空间足够时显示 `speed + subtitles + settings + theater + fullscreen`
- fullscreen 下隐藏 `theater`
- 桌面与 fullscreen 下 `next + episodes` 必须保持相邻，且相对顺序不变
- mobile touch 下采用“大播放按钮居中、next 在右侧、episodes 在左下”的布局
- `Phone Touch` 顶部空间不足时，优先级按 `settings > airplay > pip` 退化

### 完成情况（2026-04-07 20:18）
- `docs/player-feature-matrix.md` 已改为“下一轮重建目标 contract”，并新增 `Fullscreen Immersive` 维度
- 文档已补充 `AirPlay / PiP` 右上优先、`next + episodes` 邻接不变、touch phone 三点位布局，以及 settings / speed panel 的目标入口路径
- 文档已显式记录当前支线与目标 contract 的差异：`SettingsPanel` / `SpeedPanel` 待重建、episodes 面板滚动条待补、fullscreen 代码与测试待收口
- 下一步将按该 contract 重建 `SettingsPanel` / `SpeedPanel`，并为 `episodes` 面板补不破坏圆角的滚动条

### 方案补充（2026-04-07 20:24）
- `Tablet Touch` 已确认与 `Phone Touch` 共享核心触屏布局：中央大 `play`、右侧 `next`、左下 `episodes`
- `Speed` 已确认在所有终端都作为一级按钮常驻；触屏端不再通过 `SettingsPanel` 间接访问
- `Volume button` / `volume slider` 已确认仅在桌面与桌面 fullscreen 保留；移动端交由设备音量控制
- `Time display` 已确认在触屏布局中移动到进度条左端上方；手机端需使用更小字号，避免占用底部 controls
