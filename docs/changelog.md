...
- 新增 `SEQ-20260407-35`
- 本轮清理 speed/settings 重建后已废弃的旧弹窗样式与图标残留

# 2026-04-07 19:04

- 完成 `SEQ-20260407-35 / PLAYER-67`
- 已删除旧 `.ytpSettingsMenu`、旧 `.ytpSpeedPanel` 盒模型、focus trap、submenu 相关样式，以及已废弃的菜单图标导出
- speed/settings 当前只依赖新的 `ActionPanel` 结构；`EpisodesPanel` 保持原有实现不变

# 2026-04-07 19:10

- 新增 `SEQ-20260407-36`
- 本轮将 speed/settings/episodes 三类弹窗的 family 与跨终端 placement 行为固化成可观察契约

# 2026-04-07 19:13

- 完成 `SEQ-20260407-36 / PLAYER-68`
- `ActionPanel` 与 `EpisodesPanel` 已分别暴露 `data-panel-family="action"` 和 `data-panel-family="episodes"`
- 已补 desktop default / compact / phone-portrait 下的 placement 与 family 回归测试，当前弹窗系统跨终端行为拥有更明确的代码级保护
